...
def kfold_te(
    ...
    """Leak-free K-fold target encoding with Bayesian smoothing.

    Computes the smoothed mean of the target for each key value, using
    out-of-fold estimates for training rows (prevents target leakage)
    and full-data estimates for test rows.

    Args:
        ktr:   String keys for training rows.
        kte:   String keys for test rows.
        y:     Target values (aligned with ktr).
        folds: K-fold split indices.
        sm:    Smoothing factor (higher = more shrinkage toward global mean).

    Returns:
        Tuple of (train_encoded, test_encoded) arrays.
    """
    gm = y.mean()
    ...
    for tr, va in folds:
        ...
        smv = (s["mean"] * s["count"] + gm * sm) / (s["count"] + sm)
        oof.iloc[va] = ktr.iloc[va].map(smv).values
    oof = oof.fillna(gm)
...
def build(
    ...
    """Build the full feature matrix for train and test.

    Constructs spatial (lat/lon, geo-clusters), temporal (cyclical time,
    day-of-week), demand-profile (leak-free target encodings, day-48 profile),
    and context features. Train and test are processed jointly to ensure
    consistent categorical encoding.

    Args:
        train: Training DataFrame with columns [geohash, day, timestamp, demand, ...].
        test:  Test DataFrame (same columns minus demand).
        y:     Target series (train demand).
        folds: List of (train_idx, val_idx) tuples for K-fold target encoding.

    Returns:
        Xtr:      Feature matrix for training rows.
        Xte:      Feature matrix for test rows (same columns as Xtr).
        cat_cols: List of categorical column names for native-categorical models.
    """

    def base(df):
        o = pd.DataFrame(index=df.index)
        m = df["timestamp"].map(mod_of)
        o["mod"] = m
        o["hour"] = m // 60
        o["minute"] = m % 60
        a = 2 * np.pi * m / 1440.0
        o["mod_sin"], o["mod_cos"] = np.sin(a), np.cos(a)
        o["day"] = pd.to_numeric(df["day"], errors="coerce")
        gh = df["geohash"].astype(str)
        c = {g: gh_decode(g) for g in gh.dropna().unique()}
        o["gh_lat"] = gh.map(lambda g: c[g][0])
        o["gh_lon"] = gh.map(lambda g: c[g][1])
        for p in (4, 5):
            o[f"gh_pre{p}"] = gh.str.slice(0, p).astype("category").cat.codes
    ...
    rt["mod"], re["mod"] = mtr.values, mte.values
    rt["hour"], re["hour"] = mtr.values // 60, mte.values // 60
    for k in (4, 5):
    ...
    pT = piv.T.sort_index()
    rs = pT.rolling(5, center=True, min_periods=1).sum()
    rc = pT.notna().rolling(5, center=True, min_periods=1).sum()
    sx = (
        ((rs - pT.fillna(0)) / (rc - pT.notna()).replace(0, np.nan)).T.stack().to_dict()
    )
    si = pT.rolling(5, center=True, min_periods=1).mean().T.stack().to_dict()
    pd_ = prof.to_dict()
    ...
    Xtr["d48_smooth"] = lk(sx, gt, mtr.values)
    Xte["d48_smooth"] = lk(si, ge, mte.values)
    for off, nm in [(-15, "prev"), (15, "next"), (-30, "prev2"), (30, "next2")]:
        Xtr[f"d48_{nm}"] = lk(pd_, gt, mtr.values + off)
        Xte[f"d48_{nm}"] = lk(pd_, ge, mte.values + off)
    ...
    for c in [
        ...
        src_tr = (
            train.geohash.astype(str).str.slice(0, 4)
            if c == "gh4"
            else train.geohash.astype(str).str.slice(0, 5)
            if c == "gh5"
            else train[c].astype(str)
        )
        src_te = (
            test.geohash.astype(str).str.slice(0, 4)
            if c == "gh4"
            else test.geohash.astype(str).str.slice(0, 5)
            if c == "gh5"
            else test[c].astype(str)
        )
        Xtr["cat_" + c] = src_tr.values
        Xte["cat_" + c] = src_te.values
        cat_cols.append("cat_" + c)
...
if __name__ == "__main__":
    ...
    for tri, vai in folds:
        ...
        m.fit(Xtr[num].iloc[tri], y.iloc[tri])
        p += m.predict(Xte[num]) / 5
