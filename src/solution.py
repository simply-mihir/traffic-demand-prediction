...
OUT_PATH = "submission.csv"; SEED = 42
...
def base_features(df):
    ...
    out["mod"]=mod; out["hour"]=mod//60; out["minute"]=mod%60
    ang=2*np.pi*mod/1440.0; out["mod_sin"],out["mod_cos"]=np.sin(ang),np.cos(ang)
    out["day"]=pd.to_numeric(df["day"],errors="coerce")
    gh=df["geohash"].astype(str); cache={g:gh_decode(g) for g in gh.dropna().unique()}
    out["gh_lat"]=gh.map(lambda g:cache[g][0]); out["gh_lon"]=gh.map(lambda g:cache[g][1])
    for p in (4,5): out[f"gh_pre{p}"]=gh.str.slice(0,p).astype("category").cat.codes
    ...
    out["LargeVehicles"]=df["LargeVehicles"].astype(str).str.strip().str.lower().map({"allowed":1,"not allowed":0}).fillna(-1)
    out["Landmarks"]=df["Landmarks"].astype(str).str.strip().str.lower().map({"yes":1,"no":0}).fillna(-1)
...
def build_design(train,test,y,folds):
    Xtr,mod_tr=base_features(train); Xte,mod_te=base_features(test)
    ...
    for nm,fn in [("mod",mod_of)]:
        rt[nm]=train["timestamp"].map(fn); re[nm]=test["timestamp"].map(fn)
    rt["hour"]=rt["mod"]//60; re["hour"]=re["mod"]//60
    for k in (4,5):
        rt[f"gh{k}"]=train["geohash"].astype(str).str.slice(0,k); re[f"gh{k}"]=test["geohash"].astype(str).str.slice(0,k)
    specs=[["geohash"],["geohash","hour"],["geohash","mod"],["gh4"],["gh4","hour"],
           ["RoadType"],["Weather"],["geohash","RoadType"],["gh5","hour"],["RoadType","hour"],["Weather","hour"]]
    for cols in specs:
    ...
    piv=prof.unstack("mod").sort_index(axis=1); pivT=piv.T.sort_index()
    rsT=pivT.rolling(5,center=True,min_periods=1).sum(); rcT=pivT.notna().rolling(5,center=True,min_periods=1).sum()
    ...
    Xtr["d48_smooth"]=lk(sx,gtr,mod_tr.values); Xte["d48_smooth"]=lk(si,gte,mod_te.values)
    for off,nm in [(-15,"prev"),(15,"next"),(-30,"prev2"),(30,"next2")]:
        Xtr[f"d48_{nm}"]=lk(pd_,gtr,mod_tr.values+off); Xte[f"d48_{nm}"]=lk(pd_,gte,mod_te.values+off)
    ...
    gh5_tr=train["geohash"].astype(str).str.slice(0,5).values; gh5_te=test["geohash"].astype(str).str.slice(0,5).values
    night=train.assign(mod=train["timestamp"].map(mod_of),gh5=train["geohash"].astype(str).str.slice(0,5))
    night=night[night["mod"]<=120]
    g48=night[night["day"]==48]["demand"].mean()
    reg=night.groupby(["day","gh5"])["demand"].agg(["mean","count"]); reg48=night[night["day"]==48].groupby("gh5")["demand"].mean()
    dglob=night.groupby("day")["demand"].mean(); K=30.0
    def fac(day,g5):
        den=reg48.get(g5,g48); den=den if den>0 else g48
        if (day,g5) in reg.index: mn,cnt=reg.loc[(day,g5),"mean"],reg.loc[(day,g5),"count"]
        else: mn,cnt=dglob.get(day,g48),0
    ...
    Xtr["cal_factor"]=ftr; Xte["cal_factor"]=fte
    ...
    return Xtr[cols].replace([np.inf,-np.inf],np.nan), Xte[cols].replace([np.inf,-np.inf],np.nan)

def make_model(seed):
    try:
        import lightgbm as lgb
        return ("lgb", lgb.LGBMRegressor(n_estimators=3000,learning_rate=0.02,num_leaves=96,
        ...
        return ("hgbr", HistGradientBoostingRegressor(max_iter=2500,learning_rate=0.03,
            max_leaf_nodes=128,min_samples_leaf=30,l2_regularization=1.0,
            early_stopping=True,validation_fraction=0.1,random_state=seed))

def main():
    train=pd.read_csv(TRAIN_PATH); test=pd.read_csv(TEST_PATH)
    y=pd.to_numeric(train["demand"],errors="coerce"); lo,hi=y.min(),y.max()
    print("train",train.shape,"test",test.shape)
    folds=list(KFold(5,shuffle=True,random_state=SEED).split(train))
    Xtr,Xte=build_design(train,test,y,folds)
    kind,_=make_model(SEED); print("model backend:",kind,"| features:",Xtr.shape[1])
    pred=np.zeros(len(test)); n=0
    for seed in (42,7,2024):
        for tr,va in folds:
            _,m=make_model(seed); m.fit(Xtr.iloc[tr],np.log1p(y.iloc[tr]))
            pred+=np.expm1(m.predict(Xte)); n+=1
    final=np.clip(pred/n,lo,hi)
    samp=pd.read_csv(SAMPLE_PATH)
    sub=pd.DataFrame({"Index":test["Index"].values,"demand":final})[list(samp.columns)]
    assert sub.shape==(len(test),2) and list(sub.columns)==list(samp.columns)
    assert (sub["Index"].values==test["Index"].values).all() and sub["demand"].notna().all()
    sub.to_csv(OUT_PATH,index=False)
    print("wrote",OUT_PATH,sub.shape,"| mean",round(final.mean(),4)); print(sub.head())
